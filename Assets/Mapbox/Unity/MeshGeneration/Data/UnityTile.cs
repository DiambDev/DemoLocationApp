		...
		public void SetRasterData(byte[] data, bool useMipMap = true, bool useCompression = false)
{
    if (RasterDataState != TilePropertyState.Unregistered)
    {
        ...
        {
            Debug.LogWarning("Mapbox: Received null or empty raster data. Skipping tile...");
        ...
        try
        {
            if (_rasterData == null)
            {
                // Usa un tamaño temporal mínimo (1x1) para evitar error 0x0
            ...
            {
                Debug.LogWarning("Mapbox: Failed to load image data for tile.");
                return;
            }
        ...
        {
            Debug.LogError($"Mapbox: Exception while setting raster data: {ex.Message}");
        }
    }
}
